...
class RpcClient
{
    ...
    public function __call($method, $arguments)
    {
        ...
        if(0 === strpos($method, self::ASYNC_SEND_PREFIX))
        {
            ...
            {
                throw new \Exception("{$arguments[0]['site']}-{$arguments[0]['c']}-{$arguments[0]['a']}:已经发起过异步请求");
            }
        ...
        if(0 === strpos($method, self::ASYNC_RECV_PREFIX))
        {
            ...
            if(!isset(self::$asyncInstances[$instance_key]))
            {
                throw new \Exception("{$arguments[0]['site']}-{$arguments[0]['c']}-{$arguments[0]['a']}:没有发起异步请求,不能进行异步接收");
            }
