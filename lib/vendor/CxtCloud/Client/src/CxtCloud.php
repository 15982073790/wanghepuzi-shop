<?php

namespace CxtCloud\Client;

use CxtCloud\Client\Exception\ClientException;

/**
 * Class CxtCloud
 *
 * @package   CxtCloud\Client
 * @mixin     \CxtCloud\ServiceResolver
 */
class CxtCloud
{
    /**
     * Version of the Client
     */
    const VERSION = '1.0.15';

    public static $APPCODE = false;
    public static $SECRETKEY;

    /**
     * This static method can directly call the specific service.
     *
     * @param string $serviceName
     * @param array  $arguments
     *
     * @codeCoverageIgnore
     * @return object
     * @throws ClientException
     */
    public static function __callStatic($serviceName, $arguments)
    {
        $serviceName = \ucfirst($serviceName);


        $class = 'CxtCloud' . '\\' . $serviceName . '\\ModuleResolver' ;
        if (\class_exists($class)) {
            return new $class;
        }
        else{
            throw new ClientException(
                'Please install CxtCloud/sdk to support product quick access.',
                "MRSTOCK_CLOUD_SERVICE_NOT_FOUND"
            );
        }
    }

    public static function appcodeSecretKey($appcode,$secretKey)
    {
        self::$APPCODE = $appcode;
        self::$SECRETKEY = $secretKey;
    }
}