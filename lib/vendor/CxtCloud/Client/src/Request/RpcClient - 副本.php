...
namespace CxtCloud\Client\Request;
...
class RpcClient
{
     ...
     * @throws \Exception
     */
    protected function sync($method, $arguments, $tryInt = 0,$exclude_address="")
    {
        $this->sendData($method, $arguments);
        return $this->recvData($method, $arguments, $tryInt,$exclude_address);
    }
     ...
     * @param array $arguments
     */
    public function sendData($method, $arguments,$exclude_address="")
    {
        ...
        $this->setConnection($exclude_address);
    ...
    public function recvData($method = null, $arguments = null, $tryInt = 0)
    {
        ...
        $runTime = round($this->curEndTime - $this->curStartTime, 6) * 1000;
          ...
          echo "recvData   "; echo $this->curAddress;echo "\n";
           echo "recvData   ";echo $tryInt;echo "\n";
        echo "recvData   ";var_dump($ret);
        if (!$ret) {
            ...
            if (!$info['timed_out'] && $method && $tryInt < self::TRY_COUNT) {
                $tryInt++;
                return $this->sync($method, $arguments, $tryInt,$this->curAddress);
            }
     ...
     * @throws \Exception
     */
    protected function setConnection($exclude_address="")
    {
        $this->curAddress = null;
        $this->connection = null;

        if (!$this->openConnection($exclude_address)) {
            throw new \Exception("can not connect to server", -__LINE__);
        }
    }

    protected function openConnection($exclude_address="")
    {
        $addressArray = $this->parseAddressArray(self::$addressArray, []);
        // var_dump($exclude_address);
        // print_r($addressArray);

        $count=count($addressArray);
        foreach ($addressArray as $address) {
            // if(!empty($exclude_address) && $address==$exclude_address && $count>1)
            // {
            //     echo "openConnection ";echo $exclude_address;echo "\n";
            //     continue;
            // }
    ...
    protected function createConnection()
    {
        ...
        $this->curTotal = 1;
        echo "============================================\n";
        echo "createConnection       ";var_dump($this->connection);
        if ($this->connection) {
    ...
    {
        echo "============================================\n";
        echo "getConnectionFormPool   "; echo $this->curAddress ;print_r(self::$connectionPool[$this->curAddress]);echo "\n";
        //使用连接池中连接
